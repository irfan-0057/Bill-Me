...
def bills(product_type):
    if product_type not in ['pesticide', 'fertilizer', 'all']:
        return "Invalid bill type.", 404
...
def get_bills(product_type):
    ...
    try:
        if product_type == 'all':
            ...
            bills_data = db.session.query(Bill).join(BillItem).join(Product).filter(Product.product_type == product_type).group_by(Bill.id).order_by(Bill.bill_number.desc()).all()
            for bill in bills_data:
                bills_list.append({
                    'bill_number': bill.bill_number,
                    'customer_name': bill.customer_name,
        ...
        logging.info(f"Fetched {len(bills_list)} bills for display of type '{product_type}'.")
        return jsonify(bills_list)
    except Exception as e:
        logging.error(f"Error fetching bills for type '{product_type}': {e}")
        return jsonify({'error': 'Error loading bills.'}), 500
...
def view_bill(bill_number):
    try:
        ...
        bill_date = bill_header.bill_date.strftime('%Y-%m-%d')
        ...
        for item_obj, company_name, mfg_date, exp_date, batch_num, pack_size, product_type in bill_items_data:
            ...
            product_types_for_bill_type_determination.append(product_type)
        bill_type = 'general'
